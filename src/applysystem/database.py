...
# 每天重置時間表
def init_daily():
    sql = "UPDATE machine_timetable SET machine_1=NULL, machine_2=NULL, machine_3=NULL, usrname_1=NULL, usrname_2=NULL, usrname_3=NULL"
...
def add(id, usr_name):
    user_num = check_size()
    port = 20000 + 3 * user_num
    sql = "INSERT INTO docker_info (usr_id, usr_name, machine_1_port, machine_2_port, machine_3_port) VALUES (%s, %s, %s, %s, %s)"
    val = (id, usr_name, port, port + 1, port + 2)
    mycursor.execute(sql, val)
    mydb.commit()
...
def update_timetable(period, machine_no, docker_name):
    ...
    sql = "UPDATE machine_timetable SET usrname_" + str(machine_no) + '= \'' + str(usr_name) + '\' WHERE period = ' + str(period)
    mycursor.execute(sql)
    mydb.commit()
...
def update_status(period, machine_no, docker_name):
    ...
    if docker_name != "NULL":
        usr_name=docker_name[0:len(docker_name)-2]
        port = get_port(usr_name, machine_no)
...
# 返回Port
def get_port(usr_name, machine_no):
    sql = "SELECT machine_" + str(machine_no) + "_port FROM docker_info WHERE usr_name= '" + str(usr_name) +"\'"
    mycursor.execute(sql)
    myresult = mycursor.fetchall()
    return myresult[0][0]
...
def get_status_port_queries(usr_name):
    ...
    for i in range(3):
        sql = "SELECT period, machine_" + str(i + 1) + " FROM machine_timetable WHERE usrname_" + str(i + 1) + "=\'" + str(usr_name) + "\'"
        mycursor.execute(sql)
        myresult = mycursor.fetchall()
        for x in myresult:
    ...
    for res in result:
        ...
        usr_name = res[1][0:len(res[1])-2]
        port = get_port(usr_name, res[1][-1])
