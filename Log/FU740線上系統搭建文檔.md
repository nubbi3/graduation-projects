...
由於內嵌頁面需要讀取MySQL，MySQL服務的起動會在systemd後，故不能寫成系統服務。且一個終端只能運行一個flask，所以需要啟動多個終端執行。

1. 安裝gnome-terminal
   
   ```
   $ sudo aapt install gnome-terminal
   ```
...
3. 創建新的數據表
   ...
   **創建記錄預約情況數據表**
   ...
   mysql> CREATE TABLE machine_timetable(
       ...
       -> usrname_1 VARCHAR(255),
       -> usrname_2 VARCHAR(255),
       -> usrname_3 VARCHAR(255)
   ...
   **創建記錄用戶Docker信息的數據表**
   ...
   mysql> CREATE TABLE docker_info(
      ...
      -> machine_1_port INT NOT NULL,
      -> machine_2_port INT NOT NULL,
      -> machine_3_port INT NOT NULL
   ...
   - machine_1~3_port：系統生成的Docker Port
...
<html>
  ...
  <body style="background-color:rgb(248, 236, 182);">  
    <center>
    <div class="container">
      <br><span class="info">申請需知</span><br><br><br>
      該項目 (U740線上調試環境) 目前尚處於測試階段，請輸入用戶名以及SSH Key後點擊申請
      <form action="" method="post" onsubmit="document.getElementById('btn').disabled=true;">
        <br>
        用戶名：<input type="text" placeholder="請輸入用戶名" name="usr_name" value="{{ request.form.usr_name }}">
        <br>
        SSH Key：<input type="text" placeholder="請輸入SSH Key" name="ssh_key" value="{{ request.form.ssh_key }}">
        <br>
        {% if success %}
          <p class="apply"><strong>用戶 "{{ success }}" 申請成功！若要使用機器請預約時間</strong>
        {% endif %}
        {% if fail %}
          <p class="apply"><strong>系統錯誤，請重新申請</strong>
        {% endif %}

        <br>
        {% if not success %}
        <input id="btn" class="button" type="submit" value="申請" onclick="showButtonEvent()">
        {% endif %}

        {% if wrong_info %}
          <p class="apply"><strong>請輸入用戶名以及SSH Key！</strong>
        {% endif %}

        {% if existed %}
          <p class="apply"><strong>帳號 "{{ existed }}" 已存在</strong>
        {% endif %}
      </form>

      <br>
      <div id="Wait" style="display: none;">
        <span class="info2">申請中，請稍等片刻</span>
      </div>
...
<html>
  ...
  <body style="background-color:rgb(248, 236, 182);">
    ...
    <div class="container">
      {% if port %}
        <p class="info"><strong>分配成功，請使用該Port登陸Docker:</strong> {{ port }}
      {% endif %}
      <br><br>
      <input class="button" type="button" name="Submit" onclick="javascript:history.back(-1);" value="返回上一頁">
    </div>
    </center>
  </body>
</html>
...
<html>
  ...
  <body style="background-color:rgb(248, 236, 182);">
    <center>
    <div class="container">
      <p class="info"><strong>分配失敗，請檢查是否輸入正確帳號。</strong>
      <br><br>
      <input class="button" type="button" name="Submit" onclick="javascript:history.back(-1);" value="返回上一頁">
    </div>
    </center>
...
<html>
  ...
  <body style="background-color:rgb(248, 236, 182);">
    ...
    <div class="container">
      <br>
      <span class="info">預約時間段及Port查詢</span><br><br><br>
      輸入用戶名查詢
      <form action="" method="post" onsubmit="onsubmit()">
        <br>
        用戶名：<input class="dialog" type="text" placeholder="用戶名" name="usr_name" value="{{ request.form.usr_name }}">
        <br><br>
        <input id="btn" class="button" type="submit" value="查詢">
      </form>

      {% if queries %}
        <td>用戶 "{{ usr_name }}" 預約情況如下：</td><br>
        {% for query in queries %}
          <tr>
            <td>時間段：{{ query[0] }} || Port: {{ query[1] }}</td><br>
          </tr>
        {% endfor %}
      {% endif %}

      {% if no_query %}
        <p>用戶 "{{ usr_name }}" 今天沒有任何預約！</p><br>
      {% endif %}
      ...
      {% if wrong_username %}
        <p>請檢查用戶名稱 "{{ wrong_username }}" 是否正確！</p><br>
      {% endif %}
